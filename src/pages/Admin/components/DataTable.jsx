...
export function FilterBar({ def, result, query, setQuery, onApply, onClear, onExport }) {
  ...
  const activeItems = useMemo(() => {
    ...
    Object.entries(query.filters || {}).forEach(([name, value]) => {
      if (value !== undefined && value !== null && String(value) !== '') items.push({ key: `filter:${name}`, label: fieldsByName[name]?.label || name, value: String(value), clear: () => updateFilter(name, '') })
    })
    if (query.dateFrom) items.push({ key: 'dateFrom', label: 'Desde', value: query.dateFrom, clear: () => setQuery((current) => ({ ...current, dateFrom: '', page: 1 })) })
    if (query.dateTo) items.push({ key: 'dateTo', label: 'Hasta', value: query.dateTo, clear: () => setQuery((current) => ({ ...current, dateTo: '', page: 1 })) })
    if (query.sortBy) items.push({ key: 'sortBy', label: 'Orden', value: fieldsByName[query.sortBy]?.label || query.sortBy, clear: () => setQuery((current) => ({ ...current, sortBy: '', page: 1 })) })
    if (query.includeArchived) items.push({ key: 'archived', label: 'Histórico', value: 'Incluye bajas', clear: () => setQuery((current) => ({ ...current, includeArchived: false, page: 1 })) })
    return items
  // updateFilter is intentionally derived from setQuery and does not change identity semantics here.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fieldsByName, query.filters, query.dateFrom, query.dateTo, query.sortBy, query.includeArchived, setQuery])

  return (
    <div className={`admin-filter-card admin-glass-soft ${expanded ? 'is-expanded' : ''}`}>
      <div className="admin-filter-main admin-filter-main--compact">
        ...
        <button type="button" className={`admin-button admin-button--ghost admin-filter-toggle ${expanded ? 'is-active' : ''}`} onClick={() => setExpanded((current) => !current)} aria-expanded={expanded}><Filter size={15} />Filtros{activeItems.length > 0 && <span>{activeItems.length}</span>}<ChevronDown size={14} /></button>
        <button type="button" className="admin-button admin-button--primary" onClick={onApply}><Search size={15} />Aplicar</button>
        {(query.search || activeItems.length > 0) && <button type="button" className="admin-button admin-button--ghost admin-button--icon-text" onClick={onClear}><X size={15} />Limpiar</button>}
        <button type="button" className="admin-button admin-button--ghost admin-button--icon-text" onClick={onExport}><Download size={15} />CSV</button>
      </div>

      {activeItems.length > 0 && (
        <div className="admin-active-filters" aria-label="Filtros activos">
          {activeItems.map((item) => <button type="button" key={item.key} onClick={item.clear} title="Quitar filtro"><span>{item.label}</span><strong>{item.value}</strong><X size={11} /></button>)}
        </div>
      )}
      ...
      {expanded && (
        <div className="admin-filter-grid admin-filter-grid--advanced">
          ...
          <div className="admin-filter-apply"><button type="button" onClick={onApply}><Filter size={14} />Aplicar filtros</button></div>
        </div>
      )}
    </div>
  )
...
export default function DataTable({ def, result, selected, setSelected, onView, onEdit, onDuplicate, onArchive, onRestore, onDelete, onBulk, onPage, onPageSize }) {
  ...
  const viewTitle = def.special === 'campaign' ? 'Centro de control' : def.special ? 'Vista 360°' : 'Consultar'
    ...
    <>
      ...
      <div className="admin-table-shell admin-table-shell--data">
        <table className="admin-data-table">
          ...
          <tbody>
            {rows.length === 0 ? <tr><td colSpan={def.listColumns.length + 2}><div className="admin-empty-state"><strong>Sin resultados</strong><span>No hay registros que coincidan con esta búsqueda. Probá quitando filtros.</span></div></td></tr> : rows.map((row) => {
              const id = row[def.pk]
              const archived = Boolean(row.archived_at)
              const duplicable = ['settings', 'content', 'modules', 'cases', 'testimonials', 'team', 'blog', 'gallery', 'campaigns'].includes(def.key)
              return <tr key={String(id)} className={archived ? 'is-archived' : ''}><td className="admin-checkbox-cell">{canSelect && <input type="checkbox" checked={selected.has(String(id))} onChange={(event) => toggleOne(id, event.target.checked)} />}</td>{def.listColumns.map((name) => <td key={name}><CellValue field={fields[name] || { name, type: 'text' }} value={row[name]} /></td>)}<td className="admin-actions-cell"><div className="admin-row-actions"><button type="button" title={viewTitle} aria-label={viewTitle} onClick={() => onView(row)}><Eye size={15} /></button>{def.permissions.write && <button type="button" title="Modificar" aria-label="Modificar" onClick={() => onEdit(row)}><Pencil size={15} /></button>}{duplicable && <button type="button" title="Duplicar" aria-label="Duplicar" onClick={() => onDuplicate(row)}><Copy size={15} /></button>}{def.permissions.deleteMode === 'archive' && (archived ? <button type="button" title="Restaurar" aria-label="Restaurar" onClick={() => onRestore(row)}><RotateCcw size={15} /></button> : <button type="button" title="Dar de baja" aria-label="Dar de baja" className="is-danger" onClick={() => onArchive(row)}><Archive size={15} /></button>)}{def.permissions.deleteMode === 'hard' && <button type="button" title="Eliminar" aria-label="Eliminar" className="is-danger" onClick={() => onDelete(row)}><Trash2 size={15} /></button>}</div></td></tr>
            })}
          </tbody>
        </table>
      </div>
      <div className="admin-pagination"><span>Mostrando {result?.total ? ((result.page - 1) * result.pageSize) + 1 : 0}–{Math.min((result?.page || 1) * (result?.pageSize || 50), result?.total || 0)} de <strong>{result?.total || 0}</strong></span><div><select value={result?.pageSize || 50} onChange={(event) => onPageSize(Number(event.target.value))}><option value="25">25</option><option value="50">50</option><option value="100">100</option></select><button type="button" disabled={(result?.page || 1) <= 1} onClick={() => onPage((result?.page || 1) - 1)} aria-label="Página anterior"><ChevronLeft size={16} /></button><span>{result?.page || 1} / {result?.pages || 1}</span><button type="button" disabled={(result?.page || 1) >= (result?.pages || 1)} onClick={() => onPage((result?.page || 1) + 1)} aria-label="Página siguiente"><ChevronRight size={16} /></button></div></div>
    </>
  )
}
