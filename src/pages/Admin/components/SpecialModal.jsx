...
function CampaignLandingsPanel({ campaign, landings, onSetLandingStatus }) {
          ...
          return (
            <article className="admin-campaign-landing-card" key={landing.landing_id || landing.landing_key}>
              <div className="admin-campaign-landing-card__head">
                <div>
                  <strong>{landing.name || landing.landing_key}</strong>
...
export function Campaign360Modal({ data, onClose, onOpenLeads, onEditCampaign, onLead, onSetCampaignPublic, onSetLandingStatus }) {
  ...
  return <ModalFrame title={campaign.name || campaign.campaign_key} subtitle={`${campaign.campaign_key || ''} · ${campaign.status || ''}`} onClose={onClose} actions={<>{masterAction}<button type="button" className="admin-button admin-button--ghost" onClick={onEditCampaign}><Pencil size={15} />Modificar campaña</button><button type="button" className="admin-button admin-button--ghost" onClick={onOpenLeads}>Ver todos los leads</button></>}>
    ...
    <div className="admin-special-grid">
      <CampaignLandingsPanel campaign={campaign} landings={landings} onSetLandingStatus={onSetLandingStatus} />
